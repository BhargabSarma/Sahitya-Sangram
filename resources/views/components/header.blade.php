...
<header id="blur-header"
    ...
    <div x-data="{ openNav: false, openMore: false, search: '', loading: false }"
        ...
        <button @click="openNav = !openNav"
            class="lg:hidden flex items-center ml-auto text-gray-900 hover:text-indigo-600 focus:outline-none"
            aria-label="Toggle navigation">
            <svg x-show="!openNav" class="w-7 h-7" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
        ...
        <nav class="hidden lg:flex items-center space-x-8 font-medium text-gray-900">
            ...
            <a href="{{ route('author.inquiry.form') }}" class="hover:text-indigo-600">Publish With Us</a>
            ...
            <div class="relative mr-3" x-data>
                <form action="#" method="GET"
                    ...
                    <input type="text" name="q" x-model="search"
                        class="outline-none px-2 py-1 bg-transparent text-gray-700" placeholder="Search books..."
                        autocomplete="off" @input.debounce.300ms="
                               loading = true;
                ...
                <div x-show="loading"
                    class="absolute left-0 right-0 bg-white z-40 px-4 py-3 rounded shadow mt-1 text-center">
                    <i class="fa fa-spinner fa-spin text-indigo-600"></i> Searching...
                </div>
                <!-- Dropdown AJAX Results -->
                <div x-ref="dropdown" x-show="search.length && !loading"
                    class="absolute left-0 right-0 bg-white z-40 px-4 py-3 rounded shadow mt-1"
                    style="max-height: 400px; overflow-y: auto;" @click.away="search = ''"></div>
            </div>
                ...
                <div x-data="{ open: false }" class="relative ml-4">
                    <button @click="open = !open" type="button"
                        ...
                        <img src="{{ Auth::user()->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&background=0D8ABC&color=fff' }}"
                            alt="Avatar" class="w-7 h-7 rounded-full mr-2 border border-white shadow">
                    ...
                    <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-300"
                        ...
                        <a href="{{ route('profile.show') }}"
                            class="block px-5 py-2 text-gray-700 hover:bg-gray-100 transition">
            ...
            <nav class="flex flex-col space-y-2 font-medium text-gray-900 py-2">
                ...
                @auth
                    ...
                    <button @click="openMore = !openMore"
                        class="flex items-center px-3 py-2 bg-gray-200 hover:bg-gray-300 rounded-full text-gray-800 font-semibold transition mt-2">
                        <span>More</span>
                        <i class="fa fa-caret-down ml-2"></i>
                    </button>
                    <div x-show="openMore" @click.away="openMore = false"
                        ...
                        x-transition:leave-start="opacity-100 scale-100 translate-y-0"
                        x-transition:leave-end="opacity-0 scale-95 -translate-y-2"
                        class="mt-2 bg-white rounded shadow-lg border border-gray-100 py-2 z-40" style="display: none;">
                        <a href="{{ route('profile.show') }}"
                            class="block px-5 py-2 text-gray-700 hover:bg-gray-100 transition">
                            <i class="fa fa-user mr-2"></i> My Profile
                        </a>
